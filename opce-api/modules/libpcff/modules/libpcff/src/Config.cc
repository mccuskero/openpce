...
//
void Config::setAttribute( const char *name, 
                           ConfigType type,
                           OptionType opType,
...
void Config::printUsage(const char *progName) {
    ...
    cout << "-" << setw(m_NameWidth) << setiosflags(ios::left) << "v(vvvvvvvv)";
    cout << ": ";
...
void Config::printUsageHelper( const char* progName ) {
...
// verbosity level is linked to LogLevel.
// see the number of LogLevels defined in 
// LogLevel.h
bool Config::checkVerbosityOption( char* argv ) {
   ...
   else if(!strcmp(argv, "-vvvvvv")) {
       m_VerbosityLevel = 6;    
       retVal = true;
   }            
   else if(!strcmp(argv, "-vvvvvvv")) {
       m_VerbosityLevel = 7;    
       retVal = true;
   }            
   else if(!strcmp(argv, "-vvvvvvvv")) {
       m_VerbosityLevel = 8;    
       retVal = true;
   }            
   else if(!strcmp(argv, "-vvvvvvvvv")) {
       m_VerbosityLevel = 9;    
       retVal = true;
   }            
...
Config::ConfigTuple::ConfigTuple(const ConfigTuple &in) {
    ...
    m_Value = in.m_Value;  
    m_OptionEnabled = in.m_OptionEnabled;
}
...
void Config::ConfigTuple::init( const char *name, ConfigType type, const char *desc, OptionType optType, const char* val ) {
    m_Name = name;
    m_ConfigType = type;
    m_Desc = desc;
